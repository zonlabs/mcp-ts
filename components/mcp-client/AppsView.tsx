...
export function AppsView({ userSession, onSelectApp, onAction, onDeleteApp, onAddApp }: AppsViewProps) {
  ...
  const connectedApps = useMemo((): McpServer[] => {
    ...
    for (const conn of connections) {
      ...
      const navId = catalogServerId || conn.serverId || conn.sessionId;
      const matchedServer =
        (catalogServerId ? (servers.find((s) => s.id === catalogServerId) || userServers.find((s) => s.id === catalogServerId)) : undefined) ||
        userServers.find((s) => s.id === conn.serverId) ||
        servers.find((s) => s.id === conn.serverId);

      const entry: McpServer = matchedServer
        ? { ...matchedServer, id: navId }
        : {
            id: navId,
  ...
  }, [servers, userServers, connections]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground font-mono mt-0.5">
              Discover, connect and configure MCP tools for your AI
            ...
            {displayedApps.map((app) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Card Footer */}
                  {Boolean(app.categories?.[0]?.name || (isOwner && (app.createdAt || (app as any).created_at)) || (isConnected && (stored?.updatedAt || stored?.createdAt))) && (
                    <div className="flex items-center justify-between gap-2 text-[11px] font-mono text-muted-foreground/60 pt-1">
                      ...
                      ) : isConnected && (stored?.updatedAt || stored?.createdAt) ? (
                        <span className="shrink-0">
                          Connected on {new Date(stored.updatedAt || stored.createdAt!).toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                            year: "numeric",
                          })}
