...
- After load, posts a JSON-RPC notification to the parent with method **`ui/notifications/sandbox-proxy-ready`** (same string as **`SANDBOX_PROXY_READY_METHOD`** from `@modelcontextprotocol/ext-apps`, also re-exported from `@mcp-ts/client/client`).
...
```tsx
import { DEFAULT_MCP_APP_CSP } from "@mcp-ts/client/client/react";
...
import { McpAppRenderer, DEFAULT_MCP_APP_CSP } from "@mcp-ts/client/client/react";
