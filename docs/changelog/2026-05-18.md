...
<Note>
  v2.0.0 includes breaking changes. Review the relevant package and storage documentation before upgrading.
</Note>
...
Database-backed storage (Supabase, Neon, SQLite) requires running the provided migrations before deploying. See the relevant [storage backend guides](/storage-backends/overview) for setup details.
