...
interface ModelSelectorProps {
  ...
  onSelect: (id: string, model?: ModelSelectorModel) => void;
}
...
export function ModelSelector({ selectedModel, selectedModelName, onSelect }: ModelSelectorProps) {
  ...
  const displayName =
    selected?.name ||
    selectedModelName ||
    (selectedModel === "openrouter/auto" ? "Auto Router" : selectedModel) ||
    "Select model";
  const provider =
    selected?.provider ||
    (selectedModel.startsWith("openrouter/") ? "OpenRouter" : selectedModel.split("/")[0]);
  const iconUrl = provider ? getProviderIconUrl(provider) : undefined;
        ...
        {iconUrl ? (
          <img
            ...
            className={cn(
              ...
              provider && isProviderDarkInvert(provider) && "dark:invert"
            )}
          />
        ) : null}
